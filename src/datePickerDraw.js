...
const createElementPlus = (jdp, container, isYear) => {
	createElement(
		PLUS_ICON_ELM_QUERY +
			(isYear
				? jdp.options.maxDate.year === jdp.initDate.year
					? `.${DISABLE_CLASS_NAME}`
					: ""
				: jdp.options.maxDate.year === jdp.initDate.year && jdp.options.maxDate.month === jdp.initDate.month
				? `.${DISABLE_CLASS_NAME}`
				: ""),
		container,
		EVENT_CLICK_STR,
...
const createElementMinus = (jdp, container, isYear) => {
	createElement(
		MINUS_ICON_ELM_QUERY +
			(isYear
				? jdp.options.minDate.year === jdp.initDate.year
					? `.${DISABLE_CLASS_NAME}`
					: ""
				: jdp.options.minDate.year === jdp.initDate.year && jdp.options.minDate.month === jdp.initDate.month
				? `.${DISABLE_CLASS_NAME}`
				: ""),
		container,
		EVENT_CLICK_STR,
...
const renderDays = (jdp) => {
	...
	for (let i = 0; i <= maxDaysInCalendar; i++) {
		...
		const dayContainer = createElement(
			...
			null,
			null,
			toPersianDigitsIfNeeded(dayOptions.day, jdp.options.persianDigits)
		);
